...
* inherits from `ContactListStorage`, `UserPrefsStorage` and `CommandHistoryStorage` which means it can be treated as any one (if the functionality of only one is needed).
